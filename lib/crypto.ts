...
export async function encrypt(plaintext: Uint8Array, keyBytes: Uint8Array): Promise<Uint8Array> {
  ...
  const ct = await crypto.subtle.encrypt({ name: "AES-GCM", iv: nonce }, key, plaintext.buffer as ArrayBuffer);
...
export async function decrypt(data: Uint8Array, keyBytes: Uint8Array): Promise<Uint8Array> {
  ...
  return new Uint8Array(await crypto.subtle.decrypt({ name: "AES-GCM", iv: nonce }, key, ct.buffer as ArrayBuffer));
}
